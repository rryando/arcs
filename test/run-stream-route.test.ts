/**
 * GET /api/p/:slug/runs/:runId/stream — the offset-idempotent tail of one run's
 * durable event log, re-homed from the sessions route and keyed on the run
 * store instead of the session record.
 *
 * No runner is faked here, deliberately: the route never talks to the runner. It
 * reads two things — the per-run NDJSON event log on disk and the run store's
...
import { beginRun, runsIndexPath, settleRun } from "../src/web-server/run-store.js";
import { withTempDataDir } from "./helpers/temp-data-dir.js";

const WORKSPACE = "/work/demo";
const RUN_ID = "11111111-1111-4111-8111-111111111111";
/**
 * The run log's directory segment — the slug the ask route keys logs under.
 */
const SEGMENT = "demo";
/** The route as Hono registers it — the signature the token gate is walked for. */
const STREAM_PATH = "/api/p/:slug/runs/:runId/stream";
...
function appendLog(projectDir: string, segment: string, runId: string, raw: string): void {
  const path = runEventLogPath(projectDir, segment, runId);
  mkdirSync(dirname(path), { recursive: true });
  appendFileSync(path, raw, "utf-8");
}

/** Appends whole, newline-terminated lines. */
function appendLines(projectDir: string, segment: string, runId: string, lines: string[]): void {
  appendLog(projectDir, segment, runId, lines.map((line) => `${line}\n`).join(""));
}

/** A run-store claim for `runId`, as the ask route writes one (minus the pid). */
async function seedClaimedRun(projectDir: string, runId = RUN_ID): Promise<void> {
...
/** Stamps the outcome onto the run's claim, as the run write-back does. */
async function settle(
  projectDir: string,
  runId = RUN_ID,
  outcome: "success" | "error" | "timeout" | "interrupted" = "success",
): Promise<void> {
  await settleRun(projectDir, { runId, outcome });
}

function streamUrl(base: string, query = "", runId = RUN_ID): string {
  return `${base}/api/p/demo/runs/${runId}/stream${query}`;
}

/**
 * Makes the run index unreadable, and readable again, WITHOUT the path ever
 * being absent.
 *
 * The absence matters: an absent index IS a settle (the record the run would
 ...
 * `getRun` answers the SAME `undefined` a genuinely absent record answers,
 * which is precisely why the route may not settle on that answer alone — and
 * neither one ever takes the file away.
 */
interface RunIndexBlind {
  /** Present, still holding the run, and unreadable. */
  hide(): void;
  /** Byte-identical to how it started, at its original mode. */
  show(): void;
}

function blindRunIndex(projectDir: string): RunIndexBlind {
  const indexPath = runsIndexPath(projectDir);
  ...
  return {
    hide(): void {
      ...
      // parse failure — never an index that parses without the run in it.
...
async function withUnreadableRunIndex(
  projectDir: string,
  during: () => Promise<void>,
): Promise<void> {
  const blind = blindRunIndex(projectDir);
  blind.hide();
  try {
    await during();
  } finally {
...
describe("GET /api/p/:slug/runs/:runId/stream — replay after settle", () => {
  it("replays the whole log from line 0 and closes, one frame per line with its absolute offset", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      const lines = [eventLine("one"), eventLine("two"), eventLine("three")];
      appendLines(projectDir, SEGMENT, RUN_ID, lines);
      await settle(projectDir);

      const frames = await readToEnd(streamUrl(base));
  ...
  it("starts at ?from=N, never re-sending a line the caller already holds", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [
        eventLine("one"),
        eventLine("two"),
        eventLine("three"),
      ]);
      ...
      const frames = await readToEnd(streamUrl(base, "?from=2"));
  ...
  it("answers a from past the end with the end frame alone", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("one")]);
      await settle(projectDir, RUN_ID, "error");

      const frames = await readToEnd(streamUrl(base, "?from=99"));
  ...
  it("reports a truncated log on the end frame, so a consumer knows it reached a hole", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("all that fit")]);
      // What the run write-back stamps when the log hit its cap or lost bytes
      // to a short write.
      await settleRun(projectDir, { runId: RUN_ID, outcome: "success", eventLogTruncated: true });
  ...
  it("carries the harvested runtimeSessionId on the end frame — the continuation handle", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("first turn")]);
      // What the write-back stamps when the fold harvested a runtime-native
      // session id off the log (a pi/opencode/codex first turn).
      await settleRun(projectDir, {
        runId: RUN_ID,
        outcome: "success",
        runtimeSessionId: "ses_0AskRoute00000000000000000",
      });
  ...
  it("carries a typed failure code on the end frame — the CONTINUATION_LOST re-seed signal", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("fallback")]);
      await settleRun(projectDir, {
        runId: RUN_ID,
        outcome: "error",
        errorCode: "CONTINUATION_LOST",
      });
...
describe("GET .../runs/:runId/stream — live tail", () => {
  it("streams lines written after the connection opened, then closes when the run settles", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("first")]);

      const res = await fetch(streamUrl(base));
      expect(res.status).toBe(200);
      const reader = frameReader(res);
      try {
        await reader.until("the first line", (seen) => lineFrames(seen).length === 1);

        appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("second"), eventLine("third")]);
  ...
  it("tails a run whose log does not exist yet — the claim precedes the child", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      try {
        ...
        appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("spoke at last")]);
  ...
  it("keeps a live tail open across a transient index read failure, delivering the line written after it", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("before the outage")]);
      ...
      try {
        ...
        await withUnreadableRunIndex(projectDir, async () => {
        ...
        appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("after recovery")]);
  ...
  it("never settles a claimed run on an index that FLAPS unreadable while never being absent", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("before the flap")]);
      ...
      // file parses fine by then while the run was never out of it for an
      // instant.
      const flap = (async () => {
        while (flapping) {
          blind.hide();
      ...
      try {
        ...
        // time and holds the run the whole time, so nothing here is a settle.
        ...
        appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("after the flap")]);
  ...
  it("ends the tail when the index ANSWERS that the run is gone", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("one")]);
      ...
      try {
        ...
        // A readable index that no longer holds the run: nothing will ever
        // append to that log again, so this one IS a settle.
        writeFileSync(runsIndexPath(projectDir), JSON.stringify({ runs: [] }), "utf-8");
        const frames = await reader.until("the end frame", (seen) =>
          seen.some((f) => f.event === "end"),
        );
  ...
  it("never emits a trailing partial line, and emits it exactly once when it completes", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      try {
        ...
        appendLog(projectDir, SEGMENT, RUN_ID, `${partial.slice(20)}\n`);
        await settle(projectDir);
        const frames = await reader.until("the end frame", (seen) =>
          seen.some((f) => f.event === "end"),
        );
...
describe("GET .../runs/:runId/stream — offset idempotence", () => {
  it("reconnecting at the last seen offset neither duplicates nor skips a line", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      appendLines(projectDir, SEGMENT, RUN_ID, all.slice(0, 2));

      // Connection 1 — a client that dies after two lines.
      const first = frameReader(await fetch(streamUrl(base)));
      ...
      appendLines(projectDir, SEGMENT, RUN_ID, all.slice(2));
      await settle(projectDir);

      // Connection 2 — the reconnect, at the offset the client actually holds.
      const resumed = await readToEnd(streamUrl(base, `?from=${resumeAt}`));
  ...
  it("honours Last-Event-ID, so an EventSource auto-reconnect on a frozen URL does not replay", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      appendLines(projectDir, SEGMENT, RUN_ID, all);
      await settle(projectDir);

      // The browser reopens the ORIGINAL url (from=0) and replays the header.
      const frames = await readToEnd(streamUrl(base, "?from=0"), {
  ...
  it("takes the larger of ?from and Last-Event-ID, so neither source can rewind the other", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, ["a", "b", "c", "d"].map(eventLine));
      await settle(projectDir);

      const frames = await readToEnd(streamUrl(base, "?from=3"), {
  ...
  it("refuses a malformed offset rather than silently replaying from 0", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      for (const query of ["?from=-1", "?from=abc", "?from=1.5"]) {
        const res = await fetch(streamUrl(base, query));
  ...
  it("refuses every offset that is not literally digits, so the contract and the code agree", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      for (const raw of refused) {
        const res = await fetch(streamUrl(base, `?from=${raw}`));
        const body = (await res.json()) as { ok: boolean; code: string };
      ...
      const header = await fetch(streamUrl(base), {
      ...
      expect(offsets(await readToEnd(streamUrl(base, "?from=0")))).toEqual([0]);
      expect(offsets(await readToEnd(streamUrl(base, "?from=007")))).toEqual([]);
    });
  });
});
...
describe("GET .../runs/:runId/stream — holds no per-connection run state", () => {
  it("answers two concurrent tails at different offsets, each with its own slice", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      const [whole, tail] = await Promise.all([
        readToEnd(streamUrl(base)),
        readToEnd(streamUrl(base, "?from=2")),
      ]);
      ...
      expect(texts(await readToEnd(streamUrl(base)))).toEqual(all);
    });
  });
  ...
  it("survives a server restart — a new process answers the same GET with the same frames", async () => {
    await withStreamCtx(async ({ projectDir, base, restart }) => {
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, ["one", "two", "three"].map(eventLine));
      await settle(projectDir);

      const before = await readToEnd(streamUrl(base, "?from=1"));
      const rebooted = await restart();
      const after = await readToEnd(streamUrl(rebooted, "?from=1"));
...
describe("GET .../runs/:runId/stream — refusals", () => {
  it("404s a run with neither a log nor a claim, as JSON rather than an empty stream", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      const res = await fetch(streamUrl(base, "", "22222222-2222-4222-8222-222222222222"));
      const body = (await res.json()) as { ok: boolean; code: string };
      expect(res.status).toBe(404);
      expect(body.ok).toBe(false);
      expect(body.code).toBe("RUN_RUN_LOG_NOT_FOUND");
    });
  });

  it("404s an unknown project", async () => {
    await withStreamCtx(async ({ base }) => {
      const missingProject = await fetch(`${base}/api/p/nope/runs/${RUN_ID}/stream`);
  ...
  it("is a READ route: registered GET on the composed router, and open with no X-ARCS-Token", async () => {
    await withStreamCtx(async ({ projectDir, base }) => {
      ...
      await seedClaimedRun(projectDir);
      appendLines(projectDir, SEGMENT, RUN_ID, [eventLine("open to reads")]);
      await settle(projectDir);
