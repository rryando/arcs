 ...
 * it announces itself at `SessionStart`, checkpoints at every
 * `UserPromptSubmit`, closes out at `SessionEnd`, and reports its transcript
 * at `Stop` (the last checkpoint before teardown).
 *
 ...
 *
 * The response stays ARCS-shaped (`{sessionId, stagedContext?}`); turning that
 ...
 * the `directory` key: that is the metadata key the sessions UI already reads,
 * so every session renders its worktree the same way.
 ...
 * the origin that opts it into transcript mirroring. Provenance is create-only
 * in the store, so an event arriving for an ARCS-owned thread refreshes it
 * without demoting it.
 */
 ...
 * whose SessionStart never reached us, and dropping the checkpoint would cost
 * the session its only proof of life.
 */
...
async function handleHookEvent(
  ...
  if (event.hook_event_name === "UserPromptSubmit") {
    ...
    return { sessionId: checkpoint.normalizedId };
  }

  if (event.hook_event_name === "Stop") {
    // Stop is a checkpoint like UserPromptSubmit, so an unknown session is
    // auto-registered rather than dropped.
    const session = await resolveCheckpointSession(projectDir, event);
    const checkpoint = await persistCheckpoint(projectDir, session, event);
    if (event.transcript_path !== undefined && shouldMirrorTranscript(checkpoint)) {
      await mirrorSessionTranscript(projectDir, checkpoint.normalizedId, event.transcript_path);
  ...
  return { sessionId: session.normalizedId };
}
